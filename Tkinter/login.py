from tkinter import *
import psycopg2
from pass1 import get_db_config
...
def login():
    def login_database():
        ...
        cur = conn.cursor()
        cur.execute("SELECT * FROM users WHERE name=%s AND password=%s",(e1.get(), e2.get()))
        row=cur.fetchall()
        conn.close()
        print(row)
        if row!=[]:
            name=row[0][1]
            l3.config(text="user name found with name: "+name)
        else:
    ...
    l1 = Label(login_window,text="Name: ",font="times 20")
...
def signup():
    ...
    def signup_database():
        ...
        cur.execute("INSERT INTO users(name, password) Values(%s,%s)",(e1.get(),e3.get()))
